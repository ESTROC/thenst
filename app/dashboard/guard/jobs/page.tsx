...
import { Briefcase, MapPin, IndianRupee, Loader2, CheckCircle, Clock, FileText, ExternalLink } from "lucide-react";
...
export default function GuardJobBoardPage() {
  ...
  useEffect(() => {
    ...
    async function loadApplications() {
      const qApps = query(collection(db, "job_applications"), where("guardId", "==", profile?.uid));
  ...
  function isSubscriptionActive() {
    if (!profile?.guardSubscription) return false;
    if (profile.guardSubscription.status !== "active") return false;
    const expires = new Date(profile.guardSubscription.expiresAt);
  ...
  async function handleApply(job: Job) {
    ...
    if (profile?.kycStatus !== "approved") {
      toast.error("Verification Required", {
        description: "Your account must be verified (KYC Approved) before you can apply for jobs.",
      });
      return;
    }
    ...
    if (!isSubscriptionActive()) {
      if (applications.length >= 1) {
        // They used their free apply.
        setApplyingJobId(job.id as string);
        setSubscriptionModalOpen(true);
        return;
      } else {
        // First application is free
        toast.success("Enjoy your first free job application!");
      }
    }
    ...
    try {
      ...
      await createNotification(job.hrId, {
        ...
        message: `${profile!.fullName} has applied for ${job.title}.`,
        type: "success",
      ...
      // Trigger Email Notification Silently via Brevo
      sendEmail({
        to: profile!.email,
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold tracking-tight text-foreground">Available Jobs</h1>
        <p className="text-muted-foreground mt-1">Browse and apply to verified security positions from top recruiters.</p>
      </div>

      {!isSubscriptionActive() && (
        <div className="bg-primary/10 border border-primary/20 p-4 rounded-xl flex items-center justify-between shadow-sm">
          <div>
            <h3 className="font-semibold text-primary">Unlock Unlimited Applications</h3>
            <p className="text-sm text-muted-foreground mt-1">Upgrade your account to apply to as many premium security jobs as you want.</p>
          </div>
          <Button onClick={() => setSubscriptionModalOpen(true)}>View Plans</Button>
        </div>
      )}
      ...
      {jobs.length === 0 ? (
        <Card className="flex flex-col items-center justify-center p-12 bg-white/5 border-dashed">
          ...
          <p className="text-muted-foreground text-center max-w-sm mb-6">
            There are currently no open job listings. Please check back later!
          </p>
        </Card>
      ) : (
                ...
                <CardContent className="space-y-4 flex-1 flex flex-col">
                  ...
                  <div className="pt-4 border-t border-border mt-auto">
                    {hasApplied ? (
                      ...
                      >
                        {profile?.kycStatus !== "approved" ? "Verification Required" : "Apply Now"}
                      </Button>
                    )}
                  </div>
                </CardContent>
